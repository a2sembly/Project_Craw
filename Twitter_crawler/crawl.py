...
def make_basic_url(keyword, start, end):
    ...
    q = 'q=' + parse.quote(keyword)
    period = '%20until%3A' + end + '%20since%3A' + start
    src = '&src=typed_query'
    final_url = base_url + q + period + src
    return final_url

def login_to_twitter(basic_url, username, password, driver):
...
def get_twitter_postings(keyword, start, end, driver, filepath, page_sort='Latest'):
    ...
    logged_in = login_to_twitter(basic_url,Twitter_id,Twitter_pw,driver)
    if not logged_in:
        return
